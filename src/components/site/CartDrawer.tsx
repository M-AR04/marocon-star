      ...
      {open && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 right-0 bottom-0 z-[90] w-full sm:w-[440px] bg-[color:var(--sand)] border-l border-[color:var(--gold)]/30 flex flex-col"
          >
            <div className="flex items-center justify-between px-6 h-20 border-b border-[color:var(--gold)]/20">
              <div>
                <div className="text-[10px] uppercase tracking-[0.4em] text-[color:var(--gold)]">Your Ceremony</div>
                <div className="font-serif text-2xl">Cart · {count} items</div>
              </div>
            ...
            {done ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
                ...
                <div className="font-serif text-2xl">Ritual Reserved</div>
                <p className="text-sm text-[color:var(--muted-foreground)] mt-2">Demo confirmation — no real order was placed.</p>
              </div>
            ) : items.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
                ...
                <div className="font-serif text-xl">A quiet shelf, for now</div>
                <p className="text-sm text-[color:var(--muted-foreground)] mt-2 max-w-xs">
                  Begin with a single ritual. The cart will hold it like a vitrine.
                </p>
              </div>
            ) : (
              <>
                  ...
                  {items.map((i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-serif text-base leading-tight">{i.product.name}</div>
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          <div className="text-sm">
                            <motion.span key={i.qty * i.product.price} initial={{ opacity: 0.4 }} animate={{ opacity: 1 }}>
                              {(i.qty * i.product.price).toFixed(0)} JD
                            </motion.span>
                          </div>
                        </div>
                      </div>
                ...
                <div className="border-t border-[color:var(--gold)]/20 px-6 py-5 space-y-4">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>{total.toFixed(0)} JD</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-[color:var(--muted-foreground)]">Shipping · Amman</span>
                    <span className="text-[color:var(--herbal-deep)]">Complimentary</span>
                  </div>
                  <div className="hairline" />
                  <div className="flex justify-between font-serif text-xl">
                    <span>Total</span>
                    <motion.span key={total} initial={{ opacity: 0.4, y: -4 }} animate={{ opacity: 1, y: 0 }}>{total.toFixed(0)} JD</motion.span>
                  </div>
                  <button
                    onClick={checkout}
                  ...
                  >
                    {checkingOut ? "Sealing your ritual…" : "Complete the Ceremony"}
                  </button>
                  <p className="text-[10px] text-center text-[color:var(--muted-foreground)] italic">Demo experience · no real payment is taken.</p>
                </div>
              </>
            )}
          </motion.aside>
