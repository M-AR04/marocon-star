    ...
    >
      <Link to="/rituals/$slug" params={{ slug: p.slug }} className="block">
        <div className="relative overflow-hidden bg-gradient-to-b from-[color:var(--sand)] to-[color:var(--sand-deep)] aspect-[3/4]">
          {p.badge && (
            <span className="absolute top-4 left-4 z-10 px-3 py-1 text-[10px] tracking-[0.3em] uppercase bg-[color:var(--herbal-deep)]/90 text-[color:var(--sand)] backdrop-blur">
              {p.badge}
            </span>
          )}
          <img
          ...
          <button
            ...
            className="absolute bottom-4 right-4 h-11 w-11 rounded-full bg-[color:var(--sand)] border border-[color:var(--gold)]/40 flex items-center justify-center opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-500 hover:bg-[color:var(--gold)]/20"
        ...
        <div className="pt-5 pb-2">
          ...
          <div className="font-serif text-lg mt-2 leading-tight">{p.name}</div>
          <div className="font-arabic text-sm text-[color:var(--muted-foreground)] mt-1" dir="rtl">{p.nameAr}</div>
          <div className="flex items-center justify-between mt-3">
            <span className="text-sm text-[color:var(--ink)]/70">{p.ritualTime}</span>
            <span className="text-sm font-medium">{p.price} JD</span>
          </div>
        </div>
      </Link>
    </motion.div>
